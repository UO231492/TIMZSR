...
class NeighborsBase(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self, n_neighbors=None, radius=None,
                 ...
                 p=2, metric_params=None, n_jobs=None, weights='uniform'):
    #END MIRIAM
...
class KNeighborsMixin:
    ...
    def _kneighbors_reduce_func(self, dist, start,
        ...
        return result
    #BEGIN MIRIAM
    def predict(self, X=None, n_neighbors=None, return_distance=True):
        return self.kneighbors( X=X, n_neighbors=n_neighbors, return_distance=return_distance)
    ...
    def kneighbors(self, X=None, n_neighbors=None, return_distance=True):
        ...
        if return_distance:
            ...
            cont=0
            for i in dist[0]:
                nearestDist=dist[0][cont][abs(i) < tol]
                nearestNeigh_ind=neigh_ind[0][cont][abs(i) < tol] 
                
                if len(nearestDist)==0:
                    
                    result[0][0][cont] = 1/dist[0][cont]
                    result[0][1][cont]=neigh_ind[0][cont]
                else:

                    result[0][0][cont] = dist[0][cont]
                    result[0][1][cont]= neigh_ind[0][cont]
                cont=cont +1
            result=result[0]
            #END MIRIAM           
