...
from SI_model_MPLCRidge import SI_model as SI_model_MPLCRidge
from SI_model_MPLCLSVR import SI_model as SI_model_MPLCLSVR
from sklearn.metrics import mean_squared_error
from sklearn.metrics import mean_absolute_error
import numpy as np
import time
def launch_MPLC(n_folds, model, metric,X,SI,C):
    """
    ...
    if model=='Ridge':
        SI_estimator=SI_model_MPLCRidge(n_k=1)
    if model=='LSVR':
        SI_estimator=SI_model_MPLCLSVR(n_k=1)
...
for n_folds in range(folds,folds+1):
    ...
    CvSIGenerator,end_time_total=launch_MPLC(n_folds, model, metric,X,SI,C)
